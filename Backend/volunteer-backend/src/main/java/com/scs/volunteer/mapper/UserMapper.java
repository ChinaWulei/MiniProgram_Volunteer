...
public class UserMapper {
    ...
    public Optional<UserProfileVO> findProfile(Long userId) {
        List<UserProfileVO> list = jdbcTemplate.query("""
                select u.id as user_id,u.avatar_url,u.nickname,u.name,u.identity_no as volunteer_no,u.phone,
                       p.college,p.campus,p.department,p.major_class,p.skill_tags,p.available_time,p.bio,
    ...
    public void updateProfile(Long userId, UserProfileDTO dto) {
        ...
        if (count != null && count > 0) {
            jdbcTemplate.update("""
                    update volunteer_profile set college=?,campus=?,department=?,major_class=?,skill_tags=?,available_time=?,bio=?
                    where user_id=?
                    """, dto.getCollege(), dto.getCampus(), dto.getDepartment(), dto.getMajorClass(), dto.getSkillTags(), dto.getAvailableTime(), dto.getBio(), userId);
        } else {
            jdbcTemplate.update("""
                    insert into volunteer_profile(user_id,college,campus,department,major_class,skill_tags,available_time,bio,total_hours,credit_score,service_count)
                    values(?,?,?,?,?,?,?,?,0,100,0)
                    """, userId, dto.getCollege(), dto.getCampus(), dto.getDepartment(), dto.getMajorClass(), dto.getSkillTags(), dto.getAvailableTime(), dto.getBio());
        }
    }
