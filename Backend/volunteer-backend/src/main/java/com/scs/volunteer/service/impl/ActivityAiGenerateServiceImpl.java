...
public class ActivityAiGenerateServiceImpl implements ActivityAiGenerateService {
    ...
    private String inferCategory(ActivityAiGenerateRequest request) {
        if (!blank(request.getCategory())) return request.getCategory();
        String text = nullToEmpty(request.getPrompt()) + " " + nullToEmpty(request.getTitle());
        if (text.contains("环保") || text.contains("清洁") || text.contains("垃圾")) return "环保活动";
        if (text.contains("迎新") || text.contains("新生")) return "迎新活动";
